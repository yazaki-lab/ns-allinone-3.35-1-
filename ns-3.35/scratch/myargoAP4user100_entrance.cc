...
struct UserInfo {
    ...
    uint32_t entranceDoor; // どの入り口から入ってきたか
};
...
// 入り口の位置を定義（2箇所）
static std::vector<Vector> g_doorPositions = {
    Vector(5.0, 25.0, 0.0),   // 入り口1（左側の壁）
    Vector(45.0, 25.0, 0.0)   // 入り口2（右側の壁）
};

// 新規ユーザ出現管理
static uint32_t g_spawnedUsersCount = 0;
...
// 入り口付近からランダムな位置を生成（半径3m以内）
Vector GenerateRandomPositionNearDoor(uint32_t doorIndex) {
    Vector doorPos = g_doorPositions[doorIndex];
    
    std::uniform_real_distribution<double> angleDist(0.0, 2.0 * M_PI);
    std::uniform_real_distribution<double> radiusDist(0.0, 3.0); // 半径3m以内
...
std::pair<double, double> CalculatePositionVariance() {
    ...
    for (uint32_t i = 0; i < g_userInfoList.size(); ++i) {
        if (!g_userInfoList[i].isNewUser || g_spawnedUsersCount <= i) continue;
    ...
    for (uint32_t i = g_nNewUsers; i < g_userInfoList.size(); ++i) {
        positions.push_back(g_userInfoList[i].position);
    }
...
void OutputResultsToCSV(double finalSystemThroughput, double initialSystemThroughput) {
    ...
    if (!fileExists) {
        csvOutput << "Method,NewUserName,EntranceDoor,MovementRadius,RequiredThroughput,MovementDistance,ConnectedAP,FinalSystemThroughput,InitialSystemThroughput,ImprovementRate,PositionVarianceX,PositionVarianceY,TotalPositionVariance" << std::endl;
    }
    ...
    for (uint32_t i = 0; i < g_nNewUsers; ++i) {
        if (g_userInfoList[i].isNewUser && i < g_spawnedUsersCount) {
            ...
            csvOutput << "myargo," 
                     << "User" << i << ","
                     << "Door" << g_userInfoList[i].entranceDoor << ","
                     << g_movementRadius << ","
...
double CalculateSystemThroughput() {
    ...
    for (uint32_t apId = 0; apId < g_nAPs; ++apId) {
        ...
        for (uint32_t userId = 0; userId < g_userInfoList.size(); ++userId) {
            if (!g_userInfoList[userId].isNewUser || userId < g_spawnedUsersCount) {
...
double CalculateCurrentThroughput(uint32_t userId, uint32_t apId) {
    if (userId >= g_userInfoList.size() || apId >= g_nAPs) return 0.0;
...
void UpdateUserMovement() {
    ...
    for (uint32_t i = 0; i < g_spawnedUsersCount; ++i) {
        if (!g_userInfoList[i].isNewUser || !g_userMobilityModels[i] || 
    ...
    } else if (!anyMoving && g_spawnedUsersCount < g_nNewUsers) {
        // まだ全員出現していない場合は監視継続
        Simulator::Schedule(Seconds(0.2), &UpdateUserMovement);
    } else {
...
// 新規ユーザを1人ずつ出現させる関数
void SpawnNewUser() {
    if (g_spawnedUsersCount >= g_nNewUsers) {
        return;
    }
    
    uint32_t userId = g_spawnedUsersCount;
    
    // ランダムに入り口を選択
    std::uniform_int_distribution<uint32_t> doorDist(0, g_doorPositions.size() - 1);
    uint32_t doorIndex = doorDist(g_randomEngine);
    
    // 選択された入り口の近くに位置を生成
    Vector initialPos = GenerateRandomPositionNearDoor(doorIndex);
    ...
    g_staNodes->Get(userId)->AggregateObject(customMobility);
    g_userMobilityModels[userId] = customMobility;
    
    // ユーザ情報を設定
    g_userInfoList[userId].userId = userId;
    g_userInfoList[userId].position = initialPos;
    g_userInfoList[userId].initialPosition = initialPos;
    g_userInfoList[userId].entranceDoor = doorIndex;
    g_userInfoList[userId].connectedAP = g_algorithm->SelectNearestAP(initialPos);
    g_userInfoList[userId].targetPosition = initialPos;
    g_userInfoList[userId].throughput = CalculateCurrentThroughput(userId, g_userInfoList[userId].connectedAP);
    g_userInfoList[userId].throughputThreshold = g_requiredThroughput;
    g_userInfoList[userId].hasReachedThreshold = false;
    g_userInfoList[userId].isNewUser = true;
    g_userInfoList[userId].movementDistance = 0.0;
    
    // 停止コールバックを設定
    customMobility->SetShouldStopCallback([userId]() -> bool {
        if (g_userInfoList[userId].hasReachedThreshold) {
            return true;
        }
        
        Vector userPos = g_userMobilityModels[userId]->GetPosition();
        g_userInfoList[userId].position = userPos;
        
        Vector initialPos = g_userInfoList[userId].initialPosition;
        g_userInfoList[userId].movementDistance = sqrt(
            pow(userPos.x - initialPos.x, 2) + pow(userPos.y - initialPos.y, 2));
        
        if (g_userInfoList[userId].movementDistance >= g_movementRadius) {
            g_userInfoList[userId].hasReachedThreshold = true;
            OUTPUT("ユーザ" << userId << ": 移動距離制限により停止 (" 
                   << std::fixed << std::setprecision(2) << g_userInfoList[userId].movementDistance 
                   << "m)\n");
            return true;
        }
        
        if (g_algorithm) {
            APSelectionResult result = g_algorithm->SelectOptimalAPDetailed(userPos, userId);
            double currentThroughput = CalculateCurrentThroughput(userId, result.selectedAP);
            g_userInfoList[userId].throughput = currentThroughput;
            g_userInfoList[userId].connectedAP = result.selectedAP;
            
            if (currentThroughput >= g_userInfoList[userId].throughputThreshold) {
                g_userInfoList[userId].hasReachedThreshold = true;
                OUTPUT("ユーザ" << userId << ": スループット閾値達成により停止 (" 
                       << std::setprecision(2) << currentThroughput << "Mbps >= " 
                       << g_userInfoList[userId].throughputThreshold << "Mbps)\n");
                return true;
            }
        }
    ...
    // アニメーション用の色設定
    if (g_anim) {
        g_anim->UpdateNodeColor(g_staNodes->Get(userId), 255, 0, 0);
    }
    
    OUTPUT("\n[時刻 " << std::fixed << std::setprecision(1) << Simulator::Now().GetSeconds() 
           << "s] 新規ユーザ" << userId << " 出現\n");
    OUTPUT("  入り口: Door" << doorIndex << " (" << g_doorPositions[doorIndex].x 
           << ", " << g_doorPositions[doorIndex].y << ")\n");
    OUTPUT("  初期位置: (" << std::setprecision(1) << initialPos.x << ", " << initialPos.y 
           << ") -> 最寄AP" << g_userInfoList[userId].connectedAP << " 接続\n");
...
void PrintInitialState() {
    ...
    OUTPUT("入り口数: " << g_doorPositions.size() << "\n");
    for (size_t i = 0; i < g_doorPositions.size(); ++i) {
        OUTPUT("  入り口" << i << ": (" << g_doorPositions[i].x << ", " 
               << g_doorPositions[i].y << ")\n");
    }
    ...
    for (uint32_t i = g_nNewUsers; i < g_userInfoList.size(); ++i) {
        ...
        OUTPUT("既存ユーザ" << i << " 位置: (" << std::fixed << std::setprecision(1) 
               << pos.x << ", " << pos.y << ") -> 最寄AP" << g_userInfoList[i].connectedAP << " 接続\n");
    }
...
void PrintFinalResults() {
    ...
    OUTPUT("要求スループット: " << g_requiredThroughput << "Mbps\n");
    OUTPUT("環境サイズ: 50m×50m\n");
    OUTPUT("出現した新規ユーザ数: " << g_spawnedUsersCount << "/" << g_nNewUsers << "\n\n");
    
    for (uint32_t i = 0; i < g_spawnedUsersCount; ++i) {
        if (g_userInfoList[i].isNewUser) {
            Vector pos = g_userMobilityModels[i]->GetPosition();
            OUTPUT("新規ユーザ" << i << ": 入り口Door" << g_userInfoList[i].entranceDoor 
                   << ", 最終位置(" << std::fixed << std::setprecision(1) 
                   << pos.x << ", " << pos.y << "), 移動距離: " 
                   << std::setprecision(2) << g_userInfoList[i].movementDistance 
    ...
    for (uint32_t i = g_nNewUsers; i < g_userInfoList.size(); ++i) {
        Vector initialPos = g_userInfoList[i].initialPosition;
        uint32_t initialConnectedAP = g_userInfoList[i].connectedAP;
...
int main(int argc, char *argv[]) {
    ...
    g_sessionDir = outputDir + "/AP4User16_door_" + GetTimestamp();
    ...
    // 新規ユーザのスロットを確保（実際の配置はSpawnNewUserで行う）
    for (uint32_t i = 0; i < nNewUsers; ++i) {
        ...
        g_userMobilityModels[i] = nullptr; // 後で設定
    }
    ...
    for (uint32_t i = nNewUsers; i < nUsers; ++i) {
        ...
        uint32_t row = userIndex / 10;
        uint32_t col = userIndex % 10;
        
        double x = 2.5 + col * 5.0;
    ...
    for (uint32_t i = nNewUsers; i < nUsers; ++i) {
        ...
        g_userInfoList[i].entranceDoor = 0;
    }

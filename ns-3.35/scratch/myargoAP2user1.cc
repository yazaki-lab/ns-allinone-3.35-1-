...
struct APInfo {
    ...
    uint32_t channel;
    double totalThroughput;  // 追加: AP全体のスループット
};
...
struct UserInfo {
    uint32_t userId;
    Vector position;
    Vector initialPosition;  // 追加: 初期位置
...
int main(int argc, char *argv[]) {
    ...
    // STAノード用のカスタム MobilityModelを最初から設定
    ...
    g_userInfoList[0].initialPosition = g_userMobilityModels[0]->GetPosition(); // 初期位置を保存
    g_userInfoList[0].connectedAP = 0;
    g_userInfoList[0].throughput = 0.0;
    g_userInfoList[0].throughputThreshold = 60.0; // 60Mbpsの閾値
    ...
    for (uint32_t i = 0; i < g_nAPs; ++i) {
        ...
        DUAL_COUT("  AP" << i << ": 位置(" << std::setprecision(1) << apPos.x 
                  ...
                  << ", 接続ユーザー数: " << g_apInfoList[i].connectedUsers
    ...
    for (uint32_t i = 0; i < g_nUsers; ++i) {
        ...
        double initialThroughput = CalculateCurrentThroughput(i, initialResult.selectedAP);
        
        // 初期状態でのAP情報を更新
        g_apInfoList[initialResult.selectedAP].totalThroughput = initialThroughput;
        g_apInfoList[initialResult.selectedAP].connectedUsers = 1;
    }
    ...
    DUAL_COUT("\n【スループット改善評価】" << std::endl);
    DUAL_COUT("  実験前システムスループット: " << std::setprecision(2) << initialSystemThroughput << " Mbps" << std::endl);
    DUAL_COUT("  実験後システムスループット: " << std::setprecision(2) << finalSystemThroughput << " Mbps" << std::endl);
    DUAL_COUT("  スループット改善量: " << std::setprecision(2) << throughputImprovement << " Mbps" << std::endl);
    DUAL_COUT("  改善率: " << std::setprecision(1) << improvementPercentage << "%" << std::endl);

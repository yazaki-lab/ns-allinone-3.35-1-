...
int main(int argc, char *argv[]) {
    ...
    uint32_t nNewUsers = 1; // 新規未接続ユーザー
    ...
    staPositionAlloc->Add(Vector(15.0, 15.0, 0.0));   // 新規ユーザー1
    ...
    // AP1接続ユーザー（1名）
    for (uint32_t i = 0; i < 1; ++i, ++userIndex) {
        g_userInfoList[userIndex].userId = userIndex;
    ...
    uint32_t userCounts[] = {3, 1, 2, 3}; // 各APの既存接続ユーザー数
    double utilizations[] = {0.6, 0.3, 0.45, 0.8}; // チャンネル利用率
    ...
    // 結果ファイル設定
    std::string resultFileName = g_outputDir + "/ap_selection_results_AP4user16_" + g_timestamp + ".txt";
    std::string configFileName = g_outputDir + "/simulation_config_AP4user16_" + g_timestamp + ".txt";
    std::string animFileName = g_outputDir + "/animation_AP4user16_" + g_timestamp + ".xml";
